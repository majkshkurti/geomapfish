...
const projectRule = {
  test: /TestGeoMapFishPackage_geoportal\/static-ngeo\/js\/.*\.js$/,
  use: {
    loader: 'babel-loader',
    options: {
...
module.exports = {
  output: {
    path: path.resolve(__dirname, 'TestGeoMapFishPackage_geoportal/static-ngeo/build/'),
  ...
  resolve: {
    alias: {
      TestGeoMapFishPackage: path.resolve(__dirname, 'TestGeoMapFishPackage_geoportal/static-ngeo/js'),
    }
  },
  optimization: {
    splitChunks: {
